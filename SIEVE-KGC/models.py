...
class SIEVEKGCModel(nn.Module, ABC):
    """SIEVE-KGC bi-encoder with a structural evidence query branch.

    The base query and SIEVE-enhanced queries share the same query encoder.
    Each selected evidence entity creates one evidence-conditioned query, and
    the resulting vectors are averaged into a single structural query vector.
    Candidate entity representations remain query independent and can be cached.
    """
    ...
    def encode_query_batch(self, batch_dict):
        ...
        sieve_vector = None
        if "sieve_hr_token_ids" in batch_dict:
            flat = self.encode_hr(
    ...
    def forward(
        ...
        **kwargs
        ...
        sieve_vector = None
        if sieve_hr_token_ids is not None:
