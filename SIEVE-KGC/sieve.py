"""
Train-graph-only SIEVE evidence retrieval for RAA-style SimKGC integration.

Instead of adding a post-hoc positive bonus to the top-K entity scores, this module
retrieves structural evidence entities for every (head, relation) query.  The text
of each evidence entity is appended to the query, encoded by the same SimKGC
query encoder, and the resulting vectors are averaged.  This mirrors the insertion
pattern used by RAA-KGC: an auxiliary context-enhanced query branch plus an
auxiliary contrastive objective.
"""
...
class SIEVEContextBuilder:
    def __init__(
        ...
        self.enabled = bool(getattr(args, "use_sieve", True))
    ...
    def _build_statistics(self, train_examples: List[Example]):
        ...
        if self.verbose:
            logger.info(
                "RAA-style SIEVE statistics | entities=%d relations=%d triples=%d "
    ...
    def get_evidence_ids(
        ...
        come from the training graph only.
        """
    ...
    def precompute(
        ...
        if self.verbose:
            ...
            logger.info(
                "RAA-style SIEVE evidence cache | queries=%d nonempty=%d avg_pool=%.2f",
                len(evidence),
                nonempty,
                avg_size,
            )
