...
class TripletCollator:
    """Create base queries and SIEVE-enhanced evidence queries.

    All evidence-conditioned queries are flattened into one token batch.
    `sieve_owner` maps each flattened query back to the original example for
    scatter-mean aggregation in the model.
    """
    ...
    def _encode_sieve_queries(self, examples: Sequence[Example]):
        ...
        for owner, example in enumerate(examples):
            ...
            if not evidence_ids:
                # Fall back to the original query when no evidence is available.
                # This keeps the auxiliary branch well-defined.
            ...
            for entity_id in evidence_ids:
                ...
                # The relation and one evidence entity are placed in the second
                # BERT sequence, separated by [SEP].
                pair_text = "{} [SEP] structural evidence: {}".format(
